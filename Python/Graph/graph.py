...
class Graph(object):
    ...
    @staticmethod
    def erdoes_gallaii_2(dsequence):
        """ Checks if the condition of the Erdoes-Gallai inequality
            is fullfilled
    ...
    def density(self):
        ...
        given graph, and the total number of edges, the graph could have. In
        other words: It measures how close a given graph is to a complete
        graph.  method to calculate the density of a graph
        """
        g = self.__graph_dict
        V = len(g.keys())
        E = len(self.edges())
